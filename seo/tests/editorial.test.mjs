...
test('Japanese length uses characters; incomplete model responses never produce articles',()=>{
  ...
  assert.throws(()=>outputText({status:'incomplete',output:[]}));
});
test('source provenance comes only from tool annotations, not arbitrary generated URLs',()=>{
  const r=response('invented https://evil.example',[
    {type:'url_citation',url:'https://example.org/a',title:'A'},
    {type:'url_citation',url:'https://example.org/a',title:'A'},
    {type:'url_citation',url:'http://localhost:8000',title:'No'},
  ]);
...
test('provider reserves quota before request and never retries a paid timeout',async()=>{
  ...
  const call=provider(db,jobId,{SEO_OPENAI_MODEL:'configured-model',OPENAI_API_KEY:'test-key'},async()=>{attempted++;order.push('network');throw new Error('timeout');});
  await assert.rejects(call('research','instructions','input'));
  assert.equal(attempted,1);assert.deepEqual(order,['reserve','network','log']);
  const blocked=provider({rpc:async()=>{throw new Error('quota');}},jobId,{SEO_OPENAI_MODEL:'x',OPENAI_API_KEY:'x'},async()=>{throw new Error('must not call');});
...
test('generation goes through research, writing and review with real provenance contract',async()=>{
  ...
  const call=async(stage)=>{
    stages.push(stage);
    if(stage==='research') return {value:'A sourced factual brief',response:response('brief',d.sources.map(s=>({type:'url_citation',url:s.url,title:s.title})))};
    if(stage==='write') return {value:d};
    return {value:{issues:[],unsupported_claims:[],useful_to_reader:true}};
  };
  const draft=await generate(db,{id:jobId,kind:'generate',topic:d.title,language:'fr'}, {SEO_OPENAI_MODEL:'test'},call);
...
test('a refresh uses the original slug and base revision',async()=>{
  ...
  const call=async stage=>stage==='research'?{value:'brief',response:response('brief',d.sources.map(s=>({type:'url_citation',url:s.url,title:s.title})))}:{value:stage==='write'?d:{issues:[],unsupported_claims:[],useful_to_reader:true}};
