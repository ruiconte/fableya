...
export function response(value, groundingChunks=[]) {
  return {
    candidates: [{
      content: { parts: [{ text: typeof value==='string' ? value : JSON.stringify(value) }] },
      groundingMetadata: { groundingChunks: groundingChunks.map(c => ({ web: c })) },
    }],
