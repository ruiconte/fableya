...
export function sourcesFromResponse(response, checked = new Date().toISOString()) {
  ...
  for (const item of response.output ?? []) for (const part of item.content ?? []) {
    for (const a of part.annotations ?? []) if (a.type === 'url_citation' && safeUrl(a.url)) unique.set(a.url, a.title || a.url);
  }
  return [...unique].slice(0,20).map(([url,title],i) => ({ id: i+1, url, title, checked_at: checked }));
}
export function outputText(response) {
  if (response.status !== 'completed') throw new Error('Réponse IA incomplète : aucun article enregistré');
